...
def fetch_latest_emails(limit: int = 5, offset: int = 0):
    if EMAIL_PROVIDER == "gmail_workspace":
        from gmail_connector import fetch_latest_emails as _gws_fetch
        return _gws_fetch(limit=limit, offset=offset)
    if EMAIL_PROVIDER == "outlook":
        from outlook_connector import fetch_latest_emails as _outlook_fetch
...
def fetch_emails_by_subject(search_term: str, limit: int = 20) -> list[dict]:
    if EMAIL_PROVIDER == "gmail_workspace":
        from gmail_connector import fetch_emails_by_subject as _gws_fetch
        return _gws_fetch(search_term=search_term, limit=limit)
    if EMAIL_PROVIDER == "outlook":
        from outlook_connector import fetch_emails_by_subject as _outlook_fetch
...
def fetch_unseen_emails(limit: int = 20) -> list[dict]:
    if EMAIL_PROVIDER == "gmail_workspace":
        from gmail_connector import fetch_unseen_emails as _gws_fetch
        return _gws_fetch(limit=limit)
    if EMAIL_PROVIDER == "outlook":
        from outlook_connector import fetch_unseen_emails as _outlook_fetch

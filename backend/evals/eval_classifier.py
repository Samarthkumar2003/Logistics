...
def _fetch_all(columns: str) -> list[dict]:
    """Page past Supabase's 1000-row cap so eval uses the FULL table.
    A single .execute() silently truncates at 1000; loop until a short page."""
    rows: list[dict] = []
    page, size = 0, 1000
    while True:
        chunk = (
...
def load_data(embed_live: bool) -> tuple[np.ndarray, list[str]]:
    ...
    if embed_live:
        from backend.classifier.email_classifier import _embed_queries
        rows = _fetch_all("label, content")
    ...
    rows = _fetch_all(f"label, {QWEN_EMBED_COLUMN}")
